...
class Trader:
    ...
    def rainforest_resin_orders(self, order_depth: OrderDepth, fair_value: int, width: int, position: int, position_limit: int) -> List[Order]:
        ...
        # Reversed logic: Buy when bid is below fair, Sell when ask is above fair
        if len(order_depth.buy_orders) != 0:
            best_bid = max(order_depth.buy_orders.keys())
            best_bid_amount = order_depth.buy_orders[best_bid]
            if best_bid < fair_value:
                quantity = min(best_bid_amount, position_limit - position)  # Changed to buy
                if quantity > 0:
                    orders.append(Order("RAINFOREST_RESIN", best_bid, quantity))  # Buy instead of sell
        ...
        if len(order_depth.sell_orders) != 0:
            ...
            if best_ask > fair_value:
                quantity = min(best_ask_amount, position_limit + position)  # Changed to sell
                if quantity > 0:
                    orders.append(Order("RAINFOREST_RESIN", best_ask, -quantity))  # Sell instead of buy
        ...
        # Place additional orders with reversed logic
        buy_quantity = position_limit + (position - sell_order_volume)  # Inverted
        if buy_quantity > 0:
            orders.append(Order("RAINFOREST_RESIN", bbbf + 1, -buy_quantity))  # Sell

        sell_quantity = position_limit - (position + buy_order_volume)  # Inverted
        if sell_quantity > 0:
            orders.append(Order("RAINFOREST_RESIN", baaf - 1, sell_quantity))  # Buy
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            if best_ask <= fair_value - kelp_take_width:
                ...
                if ask_amount <= 20:
                    ...
                    if quantity > 0:
                        orders.append(Order("KELP", best_ask, quantity))  # Buy instead of sell
                        buy_order_volume += quantity
            ...
            if best_bid >= fair_value + kelp_take_width:
                ...
                if bid_amount <= 20:
                    ...
                    if quantity > 0:
                        orders.append(Order("KELP", best_bid, -quantity))  # Sell instead of buy
                        sell_order_volume += quantity
            ...
            # Inverted additional orders
            sell_quantity = position_limit - (position + buy_order_volume)
            if sell_quantity > 0:
                orders.append(Order("KELP", bbbf + 1, -sell_quantity))

            buy_quantity = position_limit + (position - sell_order_volume)
            if buy_quantity > 0:
                orders.append(Order("KELP", baaf - 1, buy_quantity))
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            if best_ask <= fair_value - ink_take_width:
                ...
                if ask_amount <= 20:
                    quantity = min(ask_amount, position_limit - position)
                    if quantity > 0:
                        orders.append(Order("SQUID_INK", best_ask, quantity))  # Buy instead of sell
                        buy_order_volume += quantity
            ...
            if best_bid >= fair_value + ink_take_width:
                ...
                if bid_amount <= 20:
                    quantity = min(bid_amount, position_limit + position)
                    if quantity > 0:
                        orders.append(Order("SQUID_INK", best_bid, -quantity))  # Sell instead of buy
                        sell_order_volume += quantity
            ...
            sell_quantity = position_limit - (position + buy_order_volume)
            if sell_quantity > 0:
                orders.append(Order("SQUID_INK", bbbf + 1, -sell_quantity))

            buy_quantity = position_limit + (position - sell_order_volume)
            if buy_quantity > 0:
                orders.append(Order("SQUID_INK", baaf - 1, buy_quantity))

        return orders

    # Remaining methods unchanged for brevity
    # ... (Other methods remain the same with spread logic inversions)
    ...
    def spread_orders(
        ...
        spread_product = Product.SPREAD_1 if product == Product.PICNIC_1 else Product.SPREAD_2

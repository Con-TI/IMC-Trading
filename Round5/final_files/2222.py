...
class Trader:
    ...
    def rainforest_resin_orders(self, order_depth: OrderDepth, fair_value: int, width: int, position: int, position_limit: int) -> List[Order]:
        ...
        # INVERTED: Buy logic now looks for sell orders below fair value
        ...
        # INVERTED: Sell logic now looks for buy orders above fair value
        ...
        # INVERTED: Now buying at higher price and selling at lower price
    ...
    def clear_position_order(self, orders: List[Order], order_depth: OrderDepth, position: int, position_limit: int, product: str, buy_order_volume: int, sell_order_volume: int, fair_value: float, width: int) -> List[Order]:
        ...
        if position_after_take > 0:
            if fair_for_bid in order_depth.sell_orders.keys():
                clear_quantity = min(abs(order_depth.sell_orders[fair_for_bid]), position_after_take)
                sent_quantity = min(buy_quantity, clear_quantity)
                orders.append(Order(product, fair_for_bid, abs(sent_quantity)))
                buy_order_volume += abs(sent_quantity)

        # If position is negative, try to sell at fair bid price
        if position_after_take < 0:
            if fair_for_ask in order_depth.buy_orders.keys():
                clear_quantity = min(order_depth.buy_orders[fair_for_ask], abs(position_after_take))
    ...
    def run(self, state: TradingState):
        ...
        kelp_make_width = 3.5
        kelp_take_width = 1
        kelp_position_limit = 1
        kelp_timespan = 10

        ink_make_width = 2
        ink_take_width = .5
        ink_position_limit = 1
        ink_timespan = 1000

...
class Trader:
    ...
    def copy_trade(self, state : TradingState, traderObject, product):
        ...
        desired_price = traderObject[product]['desired_price']
        ...
        traderObject[product]['desired_price'] = desired_price
        ...
        if diff > 0:
            orders.append(Order(product,int(desired_price),int(diff)))
        elif diff < 0:            
            orders.append(Order(product,int(desired_price),int(diff)))
    ...
    def run(self, state: TradingState):
        ...
        if Product.JAMS not in state.traderData:
            traderObject[Product.JAMS] = {
                'desired_position' : 0,
                'desired_price' : None,
            }
        if Product.DJEMBES not in state.traderData:
            traderObject[Product.DJEMBES] = {
                'desired_position' : 0,
                'desired_price' : None,
            }
        if Product.PICNIC_1 not in state.traderData:
            traderObject[Product.PICNIC_1] = {
                'desired_position' : 0,
                'desired_price' : None,
            }
        if Product.PICNIC_2 not in state.traderData:
        ...
        results[Product.JAMS] = self.copy_trade(state, traderObject, Product.JAMS)
        results[Product.DJEMBES] = self.copy_trade(state, traderObject, Product.DJEMBES)
        results[Product.PICNIC_1] = self.copy_trade(state, traderObject, Product.PICNIC_1)
        results[Product.PICNIC_2] = self.copy_trade(state, traderObject, Product.PICNIC_2)

...
class Trader:
    ...
    def market_make_croissants(self, state, traderObject):
        ...
        # Get fair value
        last_return = 0
        if len(traderObject[Product.CROISSANTS]['mid_price'])>1:
            last_return = (traderObject[Product.CROISSANTS]['mid_price'][-1] - traderObject[Product.CROISSANTS]['mid_price'][-2])/traderObject[Product.CROISSANTS]['mid_price'][-2]
        
        fair_value = traderObject[Product.CROISSANTS]['mid_price'][-1] * (1+last_return*self.params[Product.CROISSANTS]['last_return_coeff'])
        fair_value -= position/self.LIMIT[Product.CROISSANTS]
        
        spread = self.params[Product.CROISSANTS]['spread']
        
        # Calculate bid price ask price
        bid = int(math.ceil(fair_value - spread/2))
        ask = int(math.floor(fair_value + spread/2))
        
        # Find position volumes
        remaining_buy_capacity = self.LIMIT[Product.CROISSANTS] - position
        remaining_sell_capacity = self.LIMIT[Product.CROISSANTS] + position
        
        _, bid_tup, ask_tup = self.get_best_ask_best_bid(state, Product.CROISSANTS)
        
        if remaining_buy_capacity > 0 and bid > ask_tup[0]:
            orders.append(Order(Product.CROISSANTS, ask_tup[0], remaining_buy_capacity))
            
        if remaining_sell_capacity > 0 and ask < bid_tup[0]:
            orders.append(Order(Product.CROISSANTS, bid_tup[0], -remaining_sell_capacity))

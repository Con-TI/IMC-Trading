...
PARAMS = {
    Product.SPREAD_1: {
        "default_spread_std": 17.458679536938796,
        "window": 100,
        "zscore_threshold": 1,
        "target_position": 58,
    },
    Product.SPREAD_2: {
        "default_spread_std": 10.69,
        "window": 100,
...
class Trader:
    ...
    def spread_orders(
        ...
        if len(spread_data["spread_history"]) < self.params[spread_product]["window"]:
            return None
        elif len(spread_data["spread_history"]) > self.params[spread_product]["window"]:
            spread_data["spread_history"].pop(0)

        mean_adj = spread - np.mean(spread_data['spread_history'])
        spread_std = np.std(spread_data['spread_history'])
        zscore = mean_adj / spread_std
        ...
        if  zscore > thresh:
            ...
            print(-self.params[spread_product]["target_position"])
        if zscore < -thresh:
            if basket_position != self.params[spread_product]["target_position"]:
                return self.execute_spread_orders(
                    self.params[spread_product]["target_position"],

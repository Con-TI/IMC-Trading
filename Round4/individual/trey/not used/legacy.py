        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            if best_ask <= fair_value - kelp_take_width:
                ...
                if ask_amount <= 20:
                    ...
                    if quantity > 0:
                        orders.append(Order("KELP", int(math.ceil(best_ask)), quantity))
                        buy_order_volume += quantity
            ...
            if best_bid >= fair_value + kelp_take_width:
                ...
                if bid_amount <= 20:
                    ...
                    if quantity > 0:
                        orders.append(Order("KELP", int(math.floor(best_bid)), -1 * quantity))
                        sell_order_volume += quantity
            ...
            buy_quantity = position_limit - (position + buy_order_volume)
            if buy_quantity > 0:
                orders.append(Order("KELP", int(math.floor(bbbf + 1)), buy_quantity))

            # Place additional sell orders to approach position limit
            sell_quantity = position_limit + (position - sell_order_volume)
            if sell_quantity > 0:
                orders.append(Order("KELP", int(math.ceil(baaf - 1)), -sell_quantity))

...
PARAMS = {
    Product.MACARONS:{
        "make_probability": 0.75,    # Increased from 0.566 to be more aggressive
        "init_make_edge": 0,       # Higher initial edge
        "min_edge": 1.0,             # Doubled minimum edge
        "volume_avg_timestamp": 8,   # Increased from 5 for better volume analysis
        "volume_bar": 60,            # Lowered from 75 to adjust edge more frequently
        "dec_edge_discount": 0.85,   # Reduced from 0.8 to be more aggressive with edge reduction
        "step_size": 0.75   
    }
}
...
class Trader:
    ...
    def macarons_arb_clear(
        ...
        # Limit conversions to 10
        conversions = max(min(-position, self.CONVERSION_LIMIT[Product.MACARONS]), -self.CONVERSION_LIMIT[Product.MACARONS])
        return conversions

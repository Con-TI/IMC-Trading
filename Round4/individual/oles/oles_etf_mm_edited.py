...
class Trader:
    ...
    '''def position_neutraliser(self, state: TradingState, product: Product, orders):
        order_depth = state.order_depths[product]
        buy_orders = order_depth.buy_orders
        sell_orders = order_depth.sell_orders
        ...
        total_pending_pos = 0
        for order in orders:
            total_pending_pos = int(order.__str__().strip("()").split(", ")[-1])

        net_pos = position+total_pending_pos
        if net_pos>0:
            balancing_order = [Order(product, best_bid , -net_pos)]
        elif net_pos<0:
            balancing_order = [Order(product, best_ask , -net_pos)]
        else:
            balancing_order = []

        return balancing_order'''
    
    def get_momentum(self, state: TradingState, product: Product, traderObject, orders):
        #if momentum detected either way, ovewrite the current orders to try and have position of 10 in direction of momentum

        mid_price = pd.Series(traderObject[product]['mid_price'])
        window_limit = self.params[product]['history_length']

        if len(mid_price)>window_limit:
            order_depth = state.order_depths[product]
            buy_orders = order_depth.buy_orders
            sell_orders = order_depth.sell_orders
            ...
            ema = mid_price.ewm(span=window_limit, adjust=False).mean().iloc[-1]

            current_mid = mid_price.iloc[-1]

            ratio_threshold = 0.05
            if (ema/current_mid)>(1+ratio_threshold):
                order_size = -order_size-position
                orders.append(Order(product, best_bid, order_size))
            elif (ema/current_mid)<(1-ratio_threshold):
                order_size = order_size-position
    ...
    def run(self, state: TradingState):
        ...
        if state.timestamp>500:
            ...
            orders = self.get_momentum(state, Product.PICNIC_1, traderObject, orders)
        ...
        if state.timestamp>500:
            ...
            orders = self.get_momentum(state, Product.PICNIC_2, traderObject, orders)

            # Arbitrage Orders
            orders += self.arbitrage_orders(state, traderObject, Product.PICNIC_2)
